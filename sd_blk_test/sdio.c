...
bool sdio_read_block(uint32_t address, uint32_t *buffer) {
    ...
    dma_set_priority(DMA2, DMA_STREAM6, DMA_SxCR_PL_LOW); // TODO: check prio
    ...
    dma_disable_peripheral_increment_mode(DMA2, DMA_STREAM6);
    dma_set_transfer_mode(DMA2, DMA_STREAM6, DMA_SxCR_DIR_PERIPHERAL_TO_MEM);
    dma_set_peripheral_address(DMA2, DMA_STREAM6, (uint32_t)&SDIO_FIFO);
    dma_set_memory_address(DMA2, DMA_STREAM6, (uint32_t)buffer);
    dma_set_number_of_data(DMA2, DMA_STREAM6, 256);
    dma_channel_select(DMA2, DMA_STREAM6, DMA_SxCR_CHSEL_4);
